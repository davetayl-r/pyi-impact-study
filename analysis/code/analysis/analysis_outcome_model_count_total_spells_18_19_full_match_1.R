...
# bootstrap smd with cluster se and confidence intervals for TE
#-----------------------------------------------------

bootstrap_standardised_mean_difference <- function(data, indicies) {
  # weighted sd function
  weighted_sd <- function(x, w) {
    weighted_mean <- sum(w * x) / sum(w)
    
    sqrt(sum(w * (x - weighted_mean)^2 / (sum(w) - 1)))
  }
  ...
  # calculate smd for this sample
  boot_att <- avg_comparisons(
    boot_model,
    variables = "pyi_flag",
    vcov = ~subclass,
    newdata = subset(pyi_flag == 1)
  ...
  intervention_sd <- weighted_sd(
    x = subset(boot_data, pyi_flag == 1)$count_total_spells_18_19,
    w = subset(boot_data, pyi_flag == 1)$weights
  )
  
  comparison_sd <- weighted_sd(
    x = subset(boot_data, pyi_flag == 0)$count_total_spells_18_19,
    w = subset(boot_data, pyi_flag == 0)$weights
  )
...
boot_smd <- boot(
  data = modelling_data,
  statistic = bootstrap_standardised_mean_difference,
  R = 4999
)

# extract data
count_total_spells_18_19_poisson_model_att_smd <- mean(boot_smd$t[,1]) 
count_total_spells_18_19_poisson_model_att_smd_se <- sd(boot_smd$t[,1])
count_total_spells_18_19_poisson_model_att_smd_ci_low <- boot.ci(boot_smd, type = "bca")$bca[4]
count_total_spells_18_19_poisson_model_att_smd_ci_high <- boot.ci(boot_smd, type = "bca")$bca[5] 
...
bootstrap_standardised_mean_difference_sex <- function(data, indicies) {
  # weighted sd function
  weighted_sd <- function(x, w) {
    weighted_mean <- sum(w * x) / sum(w)
...
boot_smd_sex <- boot(
  data = modelling_data,
  statistic = bootstrap_standardised_mean_difference_sex,
  R = 4999
)

# extract information
count_total_spells_18_19_male_smd <- mean(boot_smd_sex$t[,1])
count_total_spells_18_19_female_smd <- mean(boot_smd_sex$t[,2])
count_total_spells_18_19_poisson_model_male_smd_se <- sd(boot_smd_sex$t[,1])
count_total_spells_18_19_poisson_model_female_smd_se <- sd(boot_smd_sex$t[,2])
count_total_spells_18_19_poisson_model_male_smd_ci_low <- boot.ci(boot_smd_sex, type = "bca", index = 1)$bca[4]
count_total_spells_18_19_poisson_model_male_smd_ci_high <- boot.ci(boot_smd_sex, type = "bca", index = 1)$bca[5]
count_total_spells_18_19_poisson_model_female_smd_ci_low <- boot.ci(boot_smd_sex, type = "bca", index = 2)$bca[4]
count_total_spells_18_19_poisson_model_female_smd_ci_high <- boot.ci(boot_smd_sex, type = "bca", index = 2)$bca[5]
...
bootstrap_standardised_mean_difference_aboriginal <- function(data, indicies) {
  # weighted sd function
  weighted_sd <- function(x, w) {
    weighted_mean <- sum(w * x) / sum(w)
  ...
  # aboriginal subgroup
  outcome_data_aboriginal <- boot_data |>
    filter(
      pyi_flag == 1,
      aboriginal == 1)
  ...
  outcome_data_non_aboriginal <- boot_data |>
    filter(
      pyi_flag == 1,
      aboriginal == 0)
...
boot_smd_aboriginal <- boot(
  data = modelling_data,
  statistic = bootstrap_standardised_mean_difference_aboriginal,
  R = 4999
)

# extract information
count_total_spells_18_19_aboriginal_smd <- mean(boot_smd_aboriginal$t[,1])
count_total_spells_18_19_non_aboriginal_smd <- mean(boot_smd_aboriginal$t[,2])
count_total_spells_18_19_poisson_model_aboriginal_smd_se <- sd(boot_smd_aboriginal$t[,1])
count_total_spells_18_19_poisson_model_non_aboriginal_smd_se <- sd(boot_smd_aboriginal$t[,2])
count_total_spells_18_19_poisson_model_aboriginal_smd_ci_low <- boot.ci(boot_smd_aboriginal, type = "bca", index = 1)$bca[4]
count_total_spells_18_19_poisson_model_aboriginal_smd_ci_high <- boot.ci(boot_smd_aboriginal, type = "bca", index = 1)$bca[5]
count_total_spells_18_19_poisson_model_non_aboriginal_smd_ci_low <- boot.ci(boot_smd_aboriginal, type = "bca", index = 2)$bca[4]
count_total_spells_18_19_poisson_model_non_aboriginal_smd_ci_high <- boot.ci(boot_smd_aboriginal, type = "bca", index = 2)$bca[5]
...
bootstrap_standardised_mean_difference_housing_vulnerability <- function(data, indicies) {
  # weighted sd function
  weighted_sd <- function(x, w) {
    weighted_mean <- sum(w * x) / sum(w)
  ...
  # prior homelessness subgroup
  outcome_data_prior_homelessness <- boot_data |>
    filter(
      pyi_flag == 1,
      any_housing_spell_before_18 == 1)
  
  # estimate potential outcomes under intervention conditional on being prior homelessness
  ...
  # prior homelessness subgroup
  outcome_data_no_prior_homelessness <- boot_data |>
    filter(
      pyi_flag == 1,
  ...
  # estimate potential outcomes under comparison conditional on being no prior homelessness
...
boot_smd_housing_vulnerability <- boot(
  data = modelling_data,
  statistic = bootstrap_standardised_mean_difference_housing_vulnerability,
  R = 4999
)

# extract information
count_total_spells_18_19_prior_homelessness_smd <- mean(boot_smd_housing_vulnerability$t[,1])
count_total_spells_18_19_no_prior_homelessness_smd <- mean(boot_smd_housing_vulnerability$t[,2])
count_total_spells_18_19_poisson_model_prior_homelessness_smd_se <- sd(boot_smd_housing_vulnerability$t[,1])
count_total_spells_18_19_poisson_model_no_prior_homelessness_smd_se <- sd(boot_smd_housing_vulnerability$t[,2])
count_total_spells_18_19_poisson_model_prior_homelessness_smd_ci_low <- boot.ci(boot_smd_housing_vulnerability, type = "bca", index = 1)$bca[4]
count_total_spells_18_19_poisson_model_prior_homelessness_smd_ci_high <- boot.ci(boot_smd_housing_vulnerability, type = "bca", index = 1)$bca[5]
count_total_spells_18_19_poisson_model_no_prior_homelessness_smd_ci_low <- boot.ci(boot_smd_housing_vulnerability, type = "bca", index = 2)$bca[4]
count_total_spells_18_19_poisson_model_no_prior_homelessness_smd_ci_high <- boot.ci(boot_smd_housing_vulnerability, type = "bca", index = 2)$bca[5]
